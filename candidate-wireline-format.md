...
If a POUF is expected to have many breaking changes over time, the POUF may choose to include the POUF version number in the root metadata to manage these changes over time. If this is done, the POUF should include details about how to represent this information in the metadata as well as how breaking changes to POUFs will be managed. However, it is expected that in most cases POUFs will not change frequently, so for simplicity any POUF information should be left out of TUF metadata unless otherwise specified in a POUF.
