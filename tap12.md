...
Keyids are used in TUF metadata as shorthand references for the keys themselves. They can be used to assign keys to roles and to identify
keys in signature headers. The TUF specification currently requires that these keyids be calculated using a SHA2-256 hash of the public key it
represents, as the algorithm is also used elsewhere in the TUF specification. Yet, such a rigid
requirement does not allow for a scenario in which SHA2-256 must be deprecated, such as if a security flaw is discovered in
the algorithm. Therefore, this TAP argues for greater flexibility in how keyids are
determined by removing the requirement that they be calculated only using SHA2-256.
Instead, the specification will allow metadata owners to use any method for
calculating keyids, as long as each one generated is unique within the metadata file in
which it is defined.  This
change will allow users to replace or deprecate a chosen algorithm if needed, and  give metadata owners
more flexibility in how they determine keyids, while still ensuring a fast lookup of trusted signing keys.


# Motivation

Currently, the TUF specification requires that all keyids must be the SHA2-256 hash
...
hashing properties, but for this case, the stipulation is simply a way to calculate a unique identifier employing an
algorithm already in use by the system.
...
The most important quality of the keyids used in TUF is their uniqueness. To be
...
and security. Attacks made possible when keyids are not unique include:
...
* **Keyid collision**: If root metadata lists the same keyid K for different
  snapshot and root keys, an attacker with access to the snapshot key would also
  be able to sign valid root metadata. The attacker could then list the signature in the header with K, so a
  client verifying the signature of this root metadata file would use K to
  lookup a key trusted to sign root. Instead, it would find the snapshot key and
  ...
  every keyid is associated with only a single key in each metadata file.
...
same value of the keyid, the client will only use a key that is listed in the
...
The requirement that keyids must be calculated using SHA2-256 can be removed with just minor changes to the current TUF specification process. First, the
specification wording should be changed to stipulate that the metadata owner may calculate
...
required keyid properties, i.e. “The KEYID is an identifier for each key that MUST be unique within the delegating metadata file (either root or
delegating targets metadata) and can be generated in a manner 
determined by the metadata owner.” Once this keyid is determined, it will be listed in the delegating metadata
...
find the key(s) trusted for that role in the delegating metadata. This
...
from a metadata file only for all delegations defined in that file. So
...
signature threshold. To do so, they must also use a standardized representation to keep track of all keys
used to verify a signature, as discussed above. Once
...
assign any keyids without needing to communicate it to clients. By making this
...
is not only more flexible, but it may be easily deprecated using the following process
...
determine the keyids used by that file.

As keyid deprecation is executed, it is important that the keyids within each
...
represent the same key in different metadata files, even if both files
...
threshold of unique keys to sign the metadata.
...
key from a role not trusted to sign that file. This proposal prevents
