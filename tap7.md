...
specification.  At this point, a tool that helps developers and users test that an
implementation behaves according to the TUF specification does not
...
and presents guidelines for setting up an implementation for conformance testing.

# Motivation

Up to this point, adopters of TUF who have written an implementation have tried testing
...
the metadata generated by X can be said to conform to the specifications. However, the client
...
conforms to the TUF specification.  One solution is to define, and ideally
...
download a particular package, does it correctly download both the required
top-level metadata and the requested package?  Moreover, does it do so while still
...
```Bash
$ command foo.tgz tmp/repository-files tmp/metadata tmp/targets
```
...
command-line arguments and examine the outcome.  For instance, the
tool can check the metadata saved to *tmp/metadata* and confirm that the
Snapshot, Targets, and Timestamp metadata were saved to the *tmp/metadata*
directory, and according to the Root file loaded from *tmp/metadata*
...
one provided by the conformance tool via the --repository-files command-line
option.
