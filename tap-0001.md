...
![The possible paths of the status of TAPs](https://github.com/theupdateframework/taps/blob/master/tap-0001-1.png)
...
If you are interested in assuming ownership of a TAP, send a message asking to take over, addressed to both the original author and the TAP editors.  If the original author doesn't respond to email in a timely manner, the TAP editors will make a unilateral decision (such decisions may be reversible).

# TAP Editor Responsibilities & Workflow

A TAP editor must subscribe to the TUF mailing list. All correspondence related to TAP administration should be sent (or forwarded) to this mailing list also.

For each new TAP that comes in an editor does the following:

* Read the TAP to check if it is ready: sound and complete. The ideas must make technical sense, even if they don't seem likely to be accepted.
* The title should accurately describe the content.
* Edit the TAP for language (spelling, grammar, sentence structure, etc.), markup (Markdown), code style (examples should match [TAP 3](https://github.com/theupdateframework/taps/blob/master/tap-0003.md)).

If the TAP isn't ready, an editor will send it back to the author for revision, with specific instructions.

Once the TAP is ready for the repository, a TAP editor will:

*  Assign a TAP number (almost always just the next available number).
*  Add the TAP to a local clone of [the TAP repository](https://github.com/theupdateframework/taps).
*  Commit and push the new (or updated) TAP.
*  Monitor the repository to make sure the TAP gets added to the site properly.
*  Send email back to the TAP author with next steps (post to the TUF mailing list).

Updates to existing TAPs also come in the TUF mailing list. Many TAP authors are not TUF committers yet, so TAP editors do the commits for them.

Many TAPs are written and maintained by developers with write access to the TUF codebase. The TAP editors monitor the mailing list for TAP changes, and correct any structure, grammar, spelling, or markup mistakes they see.

TAP editors don't pass judgment on TAPs.  They merely do the administrative & editorial part (which is generally a low volume task).
