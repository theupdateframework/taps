...
constructed by a user employing the TUF command-line tools, or distributed by an out-of-band bootstrap process. This file is not intended to be automatically
available / refreshed from a repository. In fact, the map file is kept on the
...
using the map file should perform the following steps:
